...
import { LeagueMemberFixturePoints, LeagueMemberPrediction, LeagueMemberWithPoints } from "@/types/leaderboard.types";
import { useQuery, UseQueryResult } from "@tanstack/react-query";

/**
 * Hook to get all predictions for members of the primary league
 */
export  const useGetLeaderboard = (): UseQueryResult<LeagueMemberPrediction[], Error> => {
  const { primaryLeague } = useLeagueStore();
  return useQuery({
    queryKey: ["leaderboard"],
    queryFn: () => leaderboardService.getLeaderboard(primaryLeague!.id),
