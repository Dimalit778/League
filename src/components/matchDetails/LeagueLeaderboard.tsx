...
export const LeagueLeaderboard = () => {
  ...
  const renderLeaderboardItem = ({ item, index }: { item: any; index: number }) => {
    ...
    return (
      <View className={`flex-row items-center justify-between p-4 rounded-xl mb-2 ${
        ...
        <View className="flex-row items-center flex-1">
          ...
          <View className="flex-1">
            <Text className={`font-semibold ${isCurrentUser ? 'text-primary' : 'text-text'}`} numberOfLines={1}>
              {item.nickname}
            </Text>
        ...
        <View className="items-center">
          <Text className={`font-bold text-xl ${isCurrentUser ? 'text-primary' : 'text-text'}`}>
            {item.points || 0}
          </Text>
