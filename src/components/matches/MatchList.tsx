...
const MatchItem = memo(({ match, onPress }: MatchItemProps) => {
  const SIZE = 30;
  const prediction = match.predictions?.[0] ?? null;

  return (
    <TouchableOpacity activeOpacity={0.85} onPress={() => onPress(match)}>
  ...
  return (
    <FlatList
      ...
      getItemLayout={(_, index) => ({
        length: 120, // Approximate height of each match item
        offset: 120 * index,
        index,
      })}
    />
