...
const MyLeagues = () => {
  ...
  const handleSetPrimary = async (league: MemberLeagueType) => {
      ...
      <FlatList
        ...
        ListEmptyComponent={() => (
          <View className="my-10 mx-3">
            <Text className="text-center text-muted text-2xl">No leagues found</Text>
          </View>
        )}
        renderItem={({ item }) => (
          <View className="my-2 px-4">
            <View className="bg-surface p-4 rounded-xl">
              <TouchableOpacity onPress={() => handleSetPrimary(item)} activeOpacity={0.8}>
                <View className="flex-row items-center">
                  ...
                  <View className="justify-end items-end">{item.is_primary && <StarIcon size={36} />}</View>
                </View>
              </TouchableOpacity>
            </View>
          </View>
