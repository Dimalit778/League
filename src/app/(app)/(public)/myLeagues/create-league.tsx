...
const CreateLeague = () => {
  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      <BackButton />
      <KeyboardAvoidingView className="flex-1 bg-background p-3" behavior="padding">
        {createLeague.isPending && <LoadingOverlay />}

        <Text className="text-2xl font-bold mb-6 text-center text-text">Enter League Details</Text>
        {/* League Name Input */}
        ...
        />
        {/* Nickname Input */}
        <Text className="text-lg font-semibold mt-6 mb-2 text-text">Your Nickname</Text>
        <InputField
          control={control}
          name="nickname"
        ...
        <Text className="text-lg font-semibold mt-6 mb-2 text-text">Number of Members</Text>
        <View className="flex-row justify-around mb-2">
          <TouchableOpacity
            onPress={() => setMembersCount(6)}
            className={`flex-1 p-4 rounded-lg border-2 mx-2 items-center ${
              membersCount === 6 ? 'border-secondary bg-surface border-2' : 'border-border border-2'
            }`}
          >
            <Text className={`text-lg font-bold ${membersCount === 6 ? 'text-secondary' : 'text-text'}`}>
              6 Members
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
          ...
          >
            <Text
              className={`text-lg font-bold ${
                membersCount === 10 ? 'text-secondary' : limits.maxMembersPerLeague < 10 ? 'text-muted' : 'text-text'
              }`}
            >
              10 Members
            </Text>
        ...
        <View className="mt-6">
          <Button title="Create League" onPress={onSubmit} variant="primary" size="lg" disabled={!isValid} />
        </View>
