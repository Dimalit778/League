...
import { useGetLeagueAndMembers, useRemoveMember, useUpdateLeague } from '@/hooks/useLeagues';
...
const EditLeagueScreen = () => {
  ...
  const { data: leagueData, isLoading, error } = useGetLeagueAndMembers(league?.id);
  ...
  const handleRemoveMember = (deletedMember: MemberLeagueType) => {
    if (!league || member?.user_id !== league.owner_id) return;
    Alert.alert('Remove Member', `Remove ${deletedMember.nickname} from this league?`, [
  ...
  const canSaveLeagueName = leagueData && trimmedLeagueName.length > 0 && trimmedLeagueName !== leagueData.name;
  ...
  const MemberCard = ({ member }: { member: MemberLeagueType }) => {
      ...
      <View className="flex-row items-center justify-between py-3 px-2 bg-surface rounded-lg border border-border">
        ...
        {!isOwner && (
          <Button
            ...
            size="sm"
            variant="error"
            onPress={() => handleRemoveMember(member as MemberLeagueType)}
          />
        )}
      </View>
    );
  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      ...
      <View className="p-4">
        <View className="bg-surface rounded-2xl border border-border p-4 mb-4">
          ...
          <View className="flex-row items-center mb-4">
            ...
            />
            <View className="flex-1">
              <Text className="text-text text-lg font-bold">{leagueData.competition?.name}</Text>
              <Text className="text-muted text-sm">{leagueData.competition?.area}</Text>
            </View>
          </View>
      ...
      <FlatList
        data={sortedMembers}
        renderItem={({ item }) => <MemberCard member={item as MemberLeagueType} />}
