...
import { MemberLeagueType, StatsType } from '@/types';

import { decode } from 'base64-arraybuffer';
import * as ImagePicker from 'expo-image-picker';

export const membersService = {
  async getMemberStats(memberId: string): Promise<StatsType> {
    if (!memberId) {
  ...
  async getMemberLeague(memberId: string) {
    const { data, error } = await supabase
      .from('league_members')
      .select('league_id, league:leagues!league_id(*, competition:competitions(*))')
      .eq('id', memberId)
      .single();
