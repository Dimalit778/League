...
describe('leagueApi', () => {
  ...
  describe('getMyLeagues', () => {
    it('fetches leagues for a user', async () => {
      const mockData = [{ id: 'l1', league: { name: 'Test' } }];
      (supabase.from as jest.Mock).mockReturnValue({
        select: jest.fn().mockReturnThis(),
        eq: jest.fn().mockReturnThis(),
        order: jest.fn().mockResolvedValue({ data: mockData, error: null }),
  ...
  describe('joinLeague', () => {
    it('calls rpc with join code and nickname', async () => {
      const mockRpc = jest.fn().mockResolvedValue({ data: 'member-id', error: null });
      (supabase as any).rpc = mockRpc;
  ...
  describe('findLeagueByJoinCode', () => {
    it('finds a league by code', async () => {
      ...
      const mockRpc = jest.fn().mockResolvedValue({ data: [mockLeague], error: null });
      (supabase as any).rpc = mockRpc;
    ...
    it('throws when league not found', async () => {
      ...
      await expect(leagueApi.findLeagueByJoinCode('INVALID')).rejects.toThrow('League not found');
    });
  });
