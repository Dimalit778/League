...
export const useGetLeagueAndMembers = (leagueId?: string | null) => {
  return useQuery({
    ...
    queryFn: leagueId
      ? () => leagueApi.getLeagueAndMembers(leagueId)
      : skipToken,
    staleTime: STALE_TIME,
  });
};
...
export const useUpdateLeagueActivation = ({
  ...
  const userId = useAuthStore((s) => s.user?.id);
  const queryClient = useQueryClient();
  const initializePrimaryLeague = usePrimaryLeagueStore((s) =>
    s.initializePrimaryLeague
  );
  ...
  return useMutation({
    ...
    onMutate: async (activeMemberIds) => {
      ...
      const previousLeagues = queryClient.getQueryData<MyLeaguesResponse>(
        leaguesKey,
      );
      const previousSummary = queryClient.getQueryData<LeagueSummary[]>(
        summaryKey,
      );
      ...
      queryClient.setQueryData<MyLeaguesResponse>(leaguesKey, (current) => {
        ...
        return {
          primaryLeague: memberships.find((league) => league.is_primary) ??
            null,
      ...
      queryClient.setQueryData<LeagueSummary[]>(
        ...
        (current) =>
          current?.map((league) => ({
            ...league,
            active: league.member_id
              ? activeIds.has(league.member_id)
              : league.active,
          })),
      );
...
export const useFindLeagueByJoinCode = (joinCode: string) => {
  ...
  return useQuery({
    ...
    queryFn: canSearch
      ? () => leagueApi.findLeagueByJoinCode(normalizedJoinCode)
      : skipToken,
  });
};
//  -- LEAGUE OPERATIONS
export const useCreateLeague = () => {
  const initializePrimaryLeague = usePrimaryLeagueStore((state) =>
    state.initializePrimaryLeague
  );
  const userId = useAuthStore((state) => state.user?.id);
  if (!userId) throw new Error("User not authenticated");
  const queryClient = useQueryClient();
