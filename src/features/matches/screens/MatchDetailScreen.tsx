...
import { useGetMatchWithPredictions } from '@/features/matches/hooks/useMatches';
...
const MatchDetailScreen = () => {
  ...
  const { data: match, isLoading } = useGetMatchWithPredictions(Number(id), leagueId ?? '');
  if (isLoading) return <LoadingOverlay />;
  if (!match) return <LoadingOverlay />;
    ...
    <View className="flex-1 bg-background">
      ...
      <View className="flex-1 mt-10 bg-background">
        <Svg height={40} width={width} viewBox={`0 0 ${width} 40`} style={{ position: 'absolute', top: -40, left: 0 }}>
          <Path d={`M 0,40 Q ${width / 2},-15 ${width},40 L ${width},40 L 0,40 Z`} fill={colors.background} />
        </Svg>
...
const MatchHeader = ({ match }: { match: MatchType }) => {
  const insets = useSafeAreaInsets();

  return (
    <View style={{ paddingTop: insets.top }}>
      <View className="p-4">
        <TouchableOpacity className="absolute top-2 left-6" onPress={() => router.dismiss()}>
