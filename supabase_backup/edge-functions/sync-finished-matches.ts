...
async function bulkUpsertMatches(
  ...
  rows: any[],
  chunkSize = 500,
): Promise<{ updated: number; errors: { start: number; message: string }[] }> {
  let updated = 0;
  const errors: { start: number; message: string }[] = [];
  for (let i = 0; i < rows.length; i += chunkSize) {
    const slice = rows.slice(i, i + chunkSize);
    try {
      const resp: any = await supabase.from("matches").upsert(slice, { onConflict: "id" }).select("id");
      if (resp?.error) {
        errors.push({ start: i, message: resp.error.message });
      ...
      errors.push({ start: i, message: msg });
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.error("sync-finished-matches error:", { reqId, message: e.message });
    return new Response(
