...
function deriveCupProgress(matches: any[]): { current_stage: string | null; current_fixture: number | null } {
  ...
  if (groupStage) {
    const played = started
      .filter((m) => m?.stage === current_stage && typeof m?.matchday === "number")
...
async function syncCompetitions(
  supabase: ReturnType<typeof createClient>,
  fdKey: string,
): Promise<{ competitionIds: Map<string, number>; errors: string[] }> {
  console.info("🏆 Step 1: Syncing competitions...");
  const competitionIds = new Map<string, number>(); // code → id
  const errors: string[] = [];

  await Promise.allSettled(
    LEAGUE_COMPETITIONS.map(async (comp) => {
      try {
        ...
        const [logo, flag] = await Promise.all([
          apiComp.emblem
            ? tryUpload(supabase, COMPETITIONS_LOGO_BUCKET, comp.code, apiComp.emblem, `${comp.code} emblem`)
            : Promise.resolve(null),
          apiComp.area?.flag
            ? tryUpload(supabase, FLAGS_BUCKET, apiComp.area.code ?? apiComp.area.name, apiComp.area.flag, `${comp.code} flag`)
            : Promise.resolve(null),
        ]);

        let current_stage: string | null = null;
        let current_fixture: number | null = null;
        let total_fixtures: number | null = null;
        ...
        if (isCup) {
          ...
          const matchPayload = await fdFetch(`${FD_BASE}/competitions/${comp.code}/matches`, fdKey);
          const matches = Array.isArray(matchPayload?.matches) ? matchPayload.matches : [];
          const matchdays = new Set(matches.map((m: any) => m?.matchday).filter((d: any) => typeof d === "number"));
...
async function syncTeams(
  ...
  const mappedTeams = await Promise.all(
    rawTeams.map(async (t) => {
      const logo = t.crest
        ? await tryUpload(supabase, TEAMS_BUCKET, String(t.id), t.crest, `team ${t.id}`)
        : null;
      return {
        id: t.id,
        name: t.name ?? null,
  ...
  return { count, errors: [...fetchErrors, ...errors] };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const allErrors = [...compErrors, ...teamErrors, ...matchErrors];
