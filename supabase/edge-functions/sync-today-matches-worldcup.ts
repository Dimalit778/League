...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (matches.length === 0) {
      return new Response(
        JSON.stringify({ success: true, updated: 0, message: "No WC matches today" }),
    ...
    return new Response(
      JSON.stringify({ success: true, updated, totalFetched: rows.length }),
      { headers: CORS_HEADERS },
    );
  } catch (err) {
    const e = err instanceof Error ? err : new Error(String(err));
    const reqId = crypto.randomUUID();
    console.error("sync-today-matches-worldcup error:", { reqId, message: e.message, stack: e.stack });
    return new Response(
