...
// GROUP_STAGE is intentionally first — knockout stages follow.
const STAGE_ORDER = [
  "PRELIMINARY_ROUND",
...
// - During GROUP_STAGE: current_stage = 'GROUP_STAGE', current_fixture = current matchday (1-6 or 1-3)
...
async function getCupProgress(
  ...
  // Find the furthest stage that has at least one non-scheduled match (i.e. started or finished)
  ...
  // If nothing is active yet, fall back to the earliest scheduled stage
  if (activeStages.size === 0) {
    for (const m of matches) {
      if (m?.stage) activeStages.add(m.stage);
    }
  }

  // Walk STAGE_ORDER and keep the furthest match
  let current_stage: string | null = null;
  for (const stage of STAGE_ORDER) {
    if (activeStages.has(stage)) current_stage = stage;
  }

  // If we're still in the group stage, find the current matchday within it
  ...
  if (current_stage === "GROUP_STAGE") {
    ...
    // Find the highest matchday that has at least one non-scheduled match
    ...
    if (playedMatchdays.size > 0) {
      current_fixture = Math.max(...playedMatchdays);
    } else {
      // Season hasn't started yet — use matchday 1
      current_fixture = 1;
    }
  }
