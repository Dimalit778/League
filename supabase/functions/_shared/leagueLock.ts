...
/** Maximum number of custom leagues a BASIC user may own and keep active. */
const BASIC_OWNED_LEAGUES_LIMIT = 3;
/** Maximum number of custom leagues a PREMIUM user may own and keep active. */
const PREMIUM_OWNED_LEAGUES_LIMIT = 5;
...
async function getOwnedLeaguesLimit(
  ...
  switch (data?.type) {
    case 'PRO':
      return PREMIUM_OWNED_LEAGUES_LIMIT;
    case 'FREE':
    default:
      return FREE_OWNED_LEAGUES_LIMIT;
  }
}
...
export async function lockLeague(
  ...
  const { error } = await supabase
    ...
    .eq('id', leagueId);
  if (error)
    throw new Error(`Failed to lock league ${leagueId}: ${error.message}`);
}
...
export async function unlockLeague(
  ...
  const { error } = await supabase
    ...
    .eq('id', leagueId);
  if (error)
    throw new Error(`Failed to unlock league ${leagueId}: ${error.message}`);
}
...
export async function applyDowngradeRules(
  ...
  if (error)
    throw new Error(
      `Failed to fetch leagues for user ${userId}: ${error.message}`,
    );
  ...
  if (toLockIds.length > 0) {
    const { error: lockError } = await supabase
      ...
      .update({
        status: 'LOCKED',
        locked_reason: 'SUBSCRIPTION_EXPIRED' as const,
      })
      .in('id', toLockIds);
    if (lockError)
      throw new Error(`Failed to lock leagues: ${lockError.message}`);
  }

  console.log(
    `Downgrade: kept league ${toKeep.id} active, locked ${toLockIds.length} leagues for user ${userId}`,
  );
}
...
export async function applyUpgradeRules(
  ...
  const ownedLeaguesLimit = await getOwnedLeaguesLimit(supabase, userId);

  // Unlock previously subscription-locked leagues up to the user's current plan limit.
  ...
  if (error)
    throw new Error(
      `Failed to fetch locked leagues for user ${userId}: ${error.message}`,
    );
  ...
  if (activeError)
    throw new Error(`Failed to count active leagues: ${activeError.message}`);

  const activeCount = activeLeagues?.length ?? 0;
  const slotsAvailable = ownedLeaguesLimit - activeCount;
  ...
  if (toUnlockIds.length > 0) {
    const { error: unlockError } = await supabase
      ...
      .in('id', toUnlockIds);
    if (unlockError)
      throw new Error(`Failed to unlock leagues: ${unlockError.message}`);
  }

  console.log(
    `Upgrade: unlocked ${toUnlock.length} leagues for user ${userId}`,
  );
}
