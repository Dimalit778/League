-- Replace competition_logo with competition_flag. CREATE OR REPLACE VIEW
-- cannot rename columns, so drop and recreate while keeping invoker
-- security and the existing SELECT grants.
drop view if exists public.member_league_summary_view;

create view public.member_league_summary_view
