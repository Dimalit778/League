-- Predictions could be inserted/updated after kickoff: the policies only
-- checked league membership. Add a time/status gate so authenticated users
-- can only write predictions while the match is still open.
